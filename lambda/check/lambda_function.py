        ...
        if db is not None and mongodb_uri:
            try:
                ...
                if result.deleted_count == 0:
                    ...
                    # Update SES receipt rule to remove this domain
                    print(f"Removing domain {domain} from SES receipt rule")
                    rule_updated = update_ses_receipt_rule(domain, action='remove')
                    if not rule_updated:
                        print(f"Warning: Failed to remove domain {domain} from SES receipt rule")
                        # Note: We don't fail the request as the domain is already deleted
                        # The rule can be manually synced later if needed
...
def update_ses_receipt_rule(domain, action='add'):
    """
    Update the SES receipt rule to add or remove a domain from the recipients list.
    
    Args:
        domain (str): The domain to add or remove
    ...
    try:
        # Get current rule
        current_rule = get_current_receipt_rule()
        
        if not current_rule:
            print(f"Receipt rule {rule_name} not found")
            return False
        ...
        if action == 'add':
            if domain_recipient not in recipients:
                recipients.append(domain_recipient)
                print(f"Adding {domain} to receipt rule recipients")
            else:
                print(f"Domain {domain} already in receipt rule")
                return True
        elif action == 'remove':
            if domain_recipient in recipients:
                recipients.remove(domain_recipient)
                print(f"Removing {domain} from receipt rule recipients")
            else:
                print(f"Domain {domain} not found in receipt rule")
                return True
        else:
        ...
        rule_update = {
            ...
            'TlsPolicy': current_rule.get('TlsPolicy', 'Optional'),
            'Recipients': recipients if recipients else None,  # None for catch-all
            'Actions': current_rule.get('Actions', []),
            'ScanEnabled': current_rule.get('ScanEnabled', True)
        }
        
        # Remove Recipients key if empty to create catch-all rule
        if not recipients:
...
def sync_all_domains_to_receipt_rule():
    """
    Sync all domains from MongoDB to the SES receipt rule.
    This ensures the receipt rule matches the current state of registered domains.
    """
    if not db:
        print("MongoDB connection not available")
        return False
    ...
    try:
        ...
        domain_configs = db['domain_configs']
        domains = domain_configs.find({}, {'domain': 1})
        
        domain_list = [doc['domain'] for doc in domains]
        ...
        rule_update = {
            'Name': f"catch-emails-{environment}",
            'Enabled': current_rule.get('Enabled', True),
            'TlsPolicy': current_rule.get('TlsPolicy', 'Optional'),
        ...
        print(f"Successfully synced {len(domain_list)} domains to receipt rule")
        return True
        
    except Exception as e:
        print(f"Error syncing domains to receipt rule: {str(e)}")
        return False
...
def lambda_handler(event, context):
    try:
        ...
        if http_method == 'POST' and raw_path.endswith('/sync'):
            # Sync all domains from MongoDB to SES receipt rule
            print("Syncing all domains to SES receipt rule")
            ...
            # Perform sync
            sync_result = sync_all_domains_to_receipt_rule()
            
            if sync_result:
                # Get count of synced domains
                domain_configs = db['domain_configs']
                domain_count = domain_configs.count_documents({})
                ...
                return {
                    ...
                    "body": json.dumps({
                        "error": "Failed to sync domains to SES receipt rule"
                    })
                }
   ...
   # Handle POST request (existing functionality)
            ...
            try:
                ...
                try:
                    ...
                    # Update SES receipt rule to include this domain
                    print(f"Updating SES receipt rule for domain: {user_domain}")
                    rule_updated = update_ses_receipt_rule(user_domain, action='add')
                    if not rule_updated:
                        print(f"Warning: Failed to update SES receipt rule for domain {user_domain}")
                        # Note: We don't fail the request as the domain is already saved
                        # The rule can be manually synced later if needed
